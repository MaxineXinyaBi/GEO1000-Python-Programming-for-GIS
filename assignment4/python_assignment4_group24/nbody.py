...
# This is the new added CSV output function
def csv_output():
    with open("nbody_simulation.csv", "w", newline='') as csvfile:
        csv_writer = csv.writer(csvfile, delimiter=';')
        #write header
        csv_writer.writerow(['body','position_x','position_y','position_z','velocity_x','velocity_y','velocity_z','mass'])
        # write content
        for name, (position,velocity,mass) in BODIES.items():
            csv_writer.writerow([name, position[0], position[1], position[2], velocity[0], velocity[1], velocity[2], mass])
