...
void csv_output(const body state[BODIES_COUNT]) {
    std::ofstream csvfile("nbody_simulation.csv");

    // Write header
    csvfile << "body;position_x;position_y;position_z;velocity_x;velocity_y;velocity_z;mass\n";

    // Write content for each body
    for (unsigned int i = 0; i < BODIES_COUNT; ++i) {
        csvfile << state[i].name << ";"
                << state[i].position.x << ";"
                << state[i].position.y << ";"
                << state[i].position.z << ";"
                << state[i].velocity.x << ";"
